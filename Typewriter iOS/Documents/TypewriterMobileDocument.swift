...
final class TypewriterMobileDocument: ReferenceFileDocument, @unchecked Sendable {
    ...
    static let readableContentTypes: [UTType] = [.typewriterDocument]
    ...
    required init(configuration: ReadConfiguration) throws {
        guard let decoded = TypewriterDocumentPackage.decode(configuration.file) else {
        ...
        metadata = decoded.metadata ?? TypewriterDocumentMetadata(
            documentID: UUID(),
            createdAt: Date(),
    ...
    private static func title(from content: NSAttributedString) -> String {
    ...
    private static func preview(from content: NSAttributedString) -> String {
    ...
    private static func paperSize(from attributes: NSDictionary?) -> CGSize {
